...
def compute_features(filename): # pass an array holding names of all files containing activity data
    ...
    for i in range(0, num_of_files):
        ...
        for x in range(1, num_of_classes):
            ...
            dataframe = dataframe.reset_index(drop=True)
            print('start1')
            # select time window and compute features for that time window
            for y in range(0, len(dataframe), 15):
                print('start')
                if (len(dataframe) - y) < 15:
                    break
                slid_window = dataframe.loc[[y,(y+15)], :]
                Ex_feat = features(slid_window)
                print('success')
                if (x == 1) and (y == 0):
                    feat = Ex_feat
                else:
